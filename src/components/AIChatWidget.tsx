    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <button
          ...
          className={`flex items-center justify-center w-14 h-14 rounded-full shadow-xl transition-transform hover:scale-105 ${
            isOpen ? "bg-primary text-neutral-bg" : "bg-accent text-white"
          }`}
        >
          {isOpen ? <PiXDuotone size={28} /> : <PiSparkleDuotone size={28} />}
        </button>
      </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 w-[350px] sm:w-[400px] max-w-[calc(100vw-3rem)] h-[500px] max-h-[calc(100vh-8rem)] bg-surface border border-border shadow-2xl rounded-2xl flex flex-col overflow-hidden z-50"
          >
            {/* Header */}
            <div className="bg-primary text-neutral-bg p-4 flex flex-col">
            ...
            <form onSubmit={handleSend} className="p-3 border-t border-border bg-surface flex gap-2">
              <input
                ...
                className="flex-1 h-10 px-4 rounded-lg border border-border focus:outline-none focus:ring-2 focus:ring-accent text-sm bg-neutral-bg/50"
              />
              <button 
                type="submit" 
